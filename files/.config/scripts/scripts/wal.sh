...
cursor="$x,$y"
swww img $image --transition-type grow --transition-pos $cursor --invert-y --transition-step 2 --transition-fps 144 --transition-duration 2
sleep 1.1
...
# Cava colors
sed -i "s/gradient_color_1 = .*/gradient_color_1 = '${color3}'/" ~/.config/cava/config
sed -i "s/gradient_color_2 = .*/gradient_color_2 = '${color1}'/" ~/.config/cava/config
pkill -USR1 cava

# Hyprlock wallpaper
sed -i "s|path = .*/\.config/swww/wallpapers/.*|path = $image|" "$HYPRLOCK_CONFIG"
