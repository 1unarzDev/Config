...
darken() {
    ...
    r=$(printf "%02x" "$(echo "scale=0; $r * $factor / 1" | bc)")
    g=$(printf "%02x" "$(echo "scale=0; $g * $factor / 1" | bc)")
    b=$(printf "%02x" "$(echo "scale=0; $b * $factor / 1" | bc)")

    echo "#$r$g$b"
}

hex_hue() {
    magick xc:"$1" -colorspace HSL -format "%[fx:round(360*u.r)]" info:
}

hex_saturation() {
    magick xc:"$1" -colorspace HSL -format "%[fx:round(100*u.g)]" info:
}

hex_lightness() {
    magick xc:"$1" -colorspace HSL -format "%[fx:round(100*u.b)]" info:
}

theme () {
    echo ".theme-dark {" >> "$theme_dir/theme.css"
    echo "  --background-primary: $(head -n 1 ~/.cache/wal/colors);" >> "$theme_dir/theme.css"
    echo "  --background-primary-alt: $(darken $(head -n 1 ~/.cache/wal/colors | tail -1) 0.7);" >> "$theme_dir/theme.css"
    echo "  --background-accent: $(head -2 ~/.cache/wal/colors | tail -1);" >> "$theme_dir/theme.css"
    echo "  --background-secondary: $(head -n 1 ~/.cache/wal/colors | tail -1);" >> "$theme_dir/theme.css"
    echo "  --background-secondary-alt: $(darken $(head -n 1 ~/.cache/wal/colors | tail -1) 0.7);" >> "$theme_dir/theme.css"
    ...
    echo "  --background-modifier-border: transparent;" >> "$theme_dir/theme.css"
    ...
    echo "  --accent-h: $(hex_hue $(head -n 5 ~/.cache/wal/colors | tail -1));" >> "$theme_dir/theme.css"
    echo "  --accent-s: $(hex_saturation $(head -n 5 ~/.cache/wal/colors | tail -1))%;" >> "$theme_dir/theme.css"
    echo "  --accent-l: $(hex_lightness $(head -n 5 ~/.cache/wal/colors | tail -1))%;" >> "$theme_dir/theme.css"
